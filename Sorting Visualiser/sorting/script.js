...
// parentDiv: container where bars representing array ele.s will be displayed.
let parentDiv = document.querySelector('.parentDiv');

// startBtn, stopBtn: Buttons to start and stop the sorting process.
let startBtn = document.querySelector('.startbtn');
let stopBtn = document.querySelector('.stopbtn');

let sound = new Audio('swap-sound.mp3');

let sorting = false; // Keeps track of whether sorting is in progress.
let currentAlgorithm = null; 



// 2. Handling User Input -: Selects input elements for array size, element input, and buttons for creating/sorting arrays.
...
// 3. Displaying Array Elements as Bars
// This function creates visual bars to represent the array.
// Each bar’s height is proportional to the array value.
// It dynamically generates and appends these elements to parentDiv.

function createBars() {
    parentDiv.innerHTML = "";

    arr.forEach((e, i) => { // e-:ele, i-:idx 
...
// Stop button allows users to cancel sorting midway. -: 
...
// Introduces delays to visualize sorting step by step. -: 
const sleep = (time) => new Promise(resolve => setTimeout(resolve, time));



// 7. Sorting Algorithms.
// Each algorithm follows these steps -: 
// Compare elements.
// Swap them if needed.
// Update the visual bars dynamically.
// Play a swap sound.
...
// swapping the heights of bars that visually represent array elements. -: 
function swapColorHeight(j, k = null) {
    ...
    // Swapping Heights -: 
    if (k != null) {
        let e2 = document.getElementById('elem' + k); // e2 is the second bar's DOM element.
        let h2 = e2.querySelector('.bar').clientHeight; // h2 is the second bar's height.

        e1.querySelector('.bar').style.height = h2 + "px";
        e2.querySelector('.bar').style.height = h1 + "px";
    } else { // Handling Single Element Update
...
// Initialization -: 
// createBars(); is called to generate the visual representation of the array.
// stopBtn.disabled = true; ensures that the stop button is initially disabled.
